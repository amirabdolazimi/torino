import { certificate, options, services, torino } from "@/core/data/footerData";
  ...
  return (
    <div className="border-t mx-auto">
      <div className="mx-auto max-w-[1440px] px-8 divide-y-2 max-sm:divide-dashed">
            ...
            {options.map((item) => (
              <div key={item.id} className="flex items-center md:w-80 gap-4">
                <Image
                  ...
                  height={120}
                  alt={item.title}
                  className="w-[71px] h-[64px] md:w-auto md:h-auto"
                />
                <div>
                  <p className="text-[14px] md:text-[26px] font-semibold">
                    {item.title}
                  </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 py-10">
          <div className="flex justify-between md:justify-start mb-4 md:mb-0  gap-10 col-span-2 md:col-span-1">
            <ul>
              <p className="text-[22px] md:text-2xl font-semibold mb-8">
                تورینو
              </p>
              {torino.map((item) => (
                <li
                  key={item.id}
                  className="md:text-lg font-normal mb-3 transition-all hover:text-primary ease-out"
                >
          ...
          <div className="flex md:flex-col-reverse justify-between items-end col-span-2 gap-5">
            ...
            <div className="flex flex-col items-center">
              <Image
                ...
                alt="تورینو"
              />
              <div className="w-full mt-4 text-sm md:text-[15px] flex items-center gap-1">
                تلفن پشتیباتی : <span>021-1111</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="border-t text-center py-2">
        developed by{" "}
        <Link className="" href={"https://github.com/amirabdolazimi"}>
          Amir Abdolazimi
        </Link>
      </div>
    </div>
